...
function generateSecurePassword(length = 32) {
  const minLen = 16;
  const maxLen = 128;
  const targetLen = Math.max(minLen, Math.min(maxLen, Number(length) || 32));
  const alphabet = 'ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz23456789!@#$%^&*()-_=+';
  try {
    const out = [];
    const bytes = crypto.randomBytes(targetLen * 2);
    for (let i = 0; i < bytes.length && out.length < targetLen; i += 1) {
      out.push(alphabet[bytes[i] % alphabet.length]);
    }
    while (out.length < targetLen) out.push(alphabet[Math.floor(Math.random() * alphabet.length)]);
    return out.join('');
  } catch (_) {
    return `pw-${Date.now()}-${Math.random().toString(36).slice(2, 14)}!`;
  }
}
...
app.post('/api/change-password', (req, res) => {
  ...
  if (!username?.trim()) return res.status(400).json({ ok: false, errors: ['username required'] });
  const ve = validateSafeName(username, 'username', MAX_USERNAME_LEN);
  if (ve) return res.status(400).json({ ok: false, errors: [ve] });
  const providedPassword = typeof newPassword === 'string' ? String(newPassword).trim() : '';
  if (providedPassword && providedPassword.length > 1024) return res.status(400).json({ ok: false, errors: ['newPassword must be at most 1024 characters'] });
  const effectivePassword = providedPassword || generateSecurePassword(32);
  ...
  runGen({
    ...
    GEN_NEW_PASSWORD: String(effectivePassword),
    // Non-interactive change-password pack: avoid TTY prompt by using the same value as file passphrase.
    // Operators can rotate again later if they want a separate passphrase.
    GEN_PASSPHRASE: String(effectivePassword),
  }, req)
    .then(({ code, stdout, stderr }) => {
      if (code === 0) {
        const parsed = parsePackFromStdout(stdout, req);
        const packExists = parsed.packFile ? resolvePackFilePath(parsed.packFile, req) : null;
        appendAuditLog(req, 'change-password', {
          username: String(username || '').trim(),
          packFile: parsed.packFile || null,
          autoGeneratedPassword: autoGenerated,
        });
        if (parsed.packFile && !packExists) {
          return res.status(500).json({
            ok: false,
            error: 'Change password completed but .enc file is missing on server path. Check runtime mount write permissions.',
            packFileMissing: true,
            packFile: parsed.packFile,
            output: stdout.slice(-2000),
          });
        }
        if (parsed.packFile) appendDownloadHistory(req, parsed.packFile, parsed.packName);
        return res.json({
          ok: true,
          message: autoGenerated
            ? 'Change password completed. A secure password was generated and packed for download.'
            : 'Change password completed. Download the updated .enc pack.',
          output: stdout.slice(-2000),
          packFile: parsed.packFile || null,
          packName: parsed.packName || null,
          downloadPath: parsed.packFile ? `/api/download/${encodeURIComponent(parsed.packFile)}` : null,
          decryptInstructions: buildDecryptInstructions(parsed.packFile, parsed.packName),
          generatedPassword: autoGenerated ? effectivePassword : null,
          autoGeneratedPassword: autoGenerated,
        });
      }
